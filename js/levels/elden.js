...
this._iframes = 0;          // invincibility frames during dodge
this._parryWindow = 0;      // active parry window
this._parryFlash = 0;

// visual enhancements
this._ambientBossPulse = null;
this._chromaticOverlay = null;
this._emberParticles = [];
this._bossStaggerTimer = 0;
...
this._chromaticOverlay.style.cssText = `
  position:fixed;inset:0;pointer-events:none;z-index:9996;opacity:0;
  background: transparent;
  box-shadow: inset 0 0 0 4px rgba(255,0,0,0.6), inset 4px 0 0 0 rgba(0,0,255,0.4);
  transition: opacity 0.06s;
`;
document.body.appendChild(this._chromaticOverlay);

// vignette — tightens as health drops
...
this._vignetteOverlay.style.cssText = `
  ...
  transition:opacity 0.8s;opacity:0.4;
`;
document.body.appendChild(this._vignetteOverlay);

// sanity flicker overlay
this._sanityOverlay = document.createElement('div');
this._sanityOverlay.style.cssText = `
  position:fixed;inset:0;pointer-events:none;z-index:9994;opacity:0;
  background:rgba(20,0,40,0.7);transition:opacity 0.05s;
`;
document.body.appendChild(this._sanityOverlay);

// ambient boss pulse light
this._ambientBossPulse = new THREE.PointLight(0xff2200, 0, 35);
this._ambientBossPulse.position.set(0, 5, 0);
this.scene.add(this._ambientBossPulse);
...
this._onKeyDown = (e) => {
  if (e.code === 'KeyF' && !this._died) {
    if (this._bossActive) this._playerSwing();
  }
  if (e.code === 'Space' && this._bossActive && !this._died) {
    e.preventDefault();
    this._doDodge();
  }
};
  ...
  onExit() {
    ...
    if (this._chromaticOverlay) { this._chromaticOverlay.remove(); this._chromaticOverlay = null; }
    if (this._ambientBossPulse) { this.scene.remove(this._ambientBossPulse); }
    if (this._vignetteOverlay) { this._vignetteOverlay.remove(); this._vignetteOverlay = null; }
if (this._sanityOverlay)   { this._sanityOverlay.remove();   this._sanityOverlay = null; }
  ...
  // try parry first if in telegraph window
  ...
  // ... rest stays identical from here
...
_hitEnemy() {
  ...
  this._bossStunTimer = 1.6;
  this._bossStaggerTimer = 0.5;
  ...
  ep.y = 0;
  this._enemy.rotation.x = -0.35;   // stagger lean-back
  this._enemy.rotation.z = (Math.random()-0.5) * 0.3;

  // spark burst at boss position
  this._spawnHitSparks(ep.x, 2.0, ep.z);

  this._hitText.style.opacity = '1';
  setTimeout(() => { this._hitText.style.opacity = '0'; }, 380);
  this.engine.audio.play('chop');
  this._weaponLight.intensity = 8;
  setTimeout(() => { this._weaponLight.intensity = 0; }, 200);
  this._screenShake = 0.5;

  if (this._bossHealth <= 0) { this._bossDefeated(); return; }
  if (this._bossHealth <= 5 && !this._phase2) {
    this._phase2 = true;
    this._enterPhase2();
  }
  this._updateHUD();
...
_enterPhase2() {
  ...
  this._torchLights.forEach(t => { t.light.intensity = 0; });

  // flash black then surge red
  this._flashOverlay.style.background = 'rgba(0,0,0,1)';
  setTimeout(() => {
    this._flashOverlay.style.background = 'rgba(80,0,0,0.9)';
  }, 350);
  setTimeout(() => {
    this._flashOverlay.style.background = 'rgba(0,0,0,0)';
    // lights flicker back on wrong
    ...
    // boss teleports BEHIND player
    const cam = this.camera.position;
    const fwd = new THREE.Vector3();
    this.camera.getWorldDirection(fwd);
    this._enemy.position.set(
      cam.x + fwd.x * 2.5,
      0,
      cam.z + fwd.z * 2.5
    );
    this._screenShake = 1.2;
  }, 900);

  // fog surges
  if (this.scene.fog) this.scene.fog.density = 0.028;

  this.engine.audio.play('deny');
  setTimeout(() => this.engine.audio.play('deny'), 180);
  setTimeout(() => this.engine.audio.play('deny'), 420);
...
_takeDamage(amount = 1) {
  ...
  if (this._iframes > 0) return;          // dodge i-frames
  ...
  this._screenShake = amount >= 2 ? 1.2 : 0.7;
  this.engine.audio.play('deny');
  setTimeout(() => this.engine.audio.play('whoosh'), 70);
  // chromatic aberration
  if (this._chromaticOverlay) {
    this._chromaticOverlay.style.opacity = '1';
    setTimeout(() => { if (this._chromaticOverlay) this._chromaticOverlay.style.opacity = '0'; }, 260);
  }
  // 1 HP heartbeat visual
  if (this._playerHealth === 1) {
    this._flashOverlay.style.background = `rgba(200,0,0,0.6)`;
    setTimeout(() => { if (this._flashOverlay) this._flashOverlay.style.background = 'rgba(0,0,0,0)'; }, 120);
  }
  ...
  _executeAttack() {
    ...
    switch(type) {
      ...
      case 'tail': {
        ...
        if (tDist < 3.8) this._takeDamage(2);
        break;
      }
      case 'throw': {
  ...
  _endAttack() {
    ...
    switch(type) {
      case 'lunge':
        this._weaponGroup.rotation.x = 0;
        this._weaponGroup.position.z = 0.55;
        this._attackCooldown = this._phase2 ? 0.5 : 0.8;
        break;
      ...
      case 'slam':
        ...
        this._attackCooldown = this._phase2 ? 0.7 : 1.1;
        break;
      ...
      case 'combo':
        ...
        if (this._comboStep < 2) {
          ...
          this._attackCooldown = this._phase2 ? 0.6 : 1.0;
        }
        break;
      ...
      case 'throw':
        ...
        this._weaponGroup.position.y = 1.3;
        this._weaponLight.intensity = 0;
        this._attackCooldown = this._phase2 ? 0.65 : 1.1;
        break;
    }
  ...
  _spawnHitSparks(x, y, z) {
  for (let i = 0; i < 8; i++) {
    const spark = new THREE.Mesh(
      new THREE.BoxGeometry(0.06, 0.06, 0.06),
      new THREE.MeshBasicMaterial({ color: i % 2 === 0 ? 0xffcc00 : 0xff6600 })
    );
...
_tryParry() {
  // called from _playerSwing when parry window is active
  if (this._parryWindow > 0 && this._currentAttack && this._attackPhase === 'telegraph') {
    this._parryWindow = 0;
    this._bossStunTimer = 3.0;
    this._bossStunned = true;
    this._attackCooldown = 3.5;
    this._currentAttack = null;
    this._attackPhase = 'idle';
    this._hideTelegraphs();
    this._showWarning('✦ PARRIED ✦');
    this._screenShake = 0.4;
    this._weaponLight.intensity = 10;
    this.engine.audio.play('cash');
    setTimeout(() => { this._weaponLight.intensity = 0; }, 300);
    const ep = this._enemy.position;
    this._spawnHitSparks(ep.x, 2.0, ep.z);
    return true;
  }
...
_doDodge() {
  if (this._dodgeCooldown > 0 || this._stamina < 0.25) return;
  this._dodgeActive = true;
  this._dodgeTimer = 0.28;
  this._iframes = 0.28;
  this._dodgeCooldown = 1.1;
  this._stamina = Math.max(0, this._stamina - 0.25);

  // get dodge direction from WASD input or default backward
  ...
  // default: dodge backward away from boss
  if (mlen < 0.01) {
    const ep = this._enemy.position;
    const bx = this.camera.position.x - ep.x;
    const bz = this.camera.position.z - ep.z;
    const bl = Math.sqrt(bx*bx+bz*bz) || 1;
    this._dodgeDir.set(bx/bl, 0, bz/bl);
  } else {
    this._dodgeDir.set(mx/mlen, 0, mz/mlen);
  }
  this.engine.audio.play('whoosh');
}

_triggerSanityFlicker() {
  // random flicker sequence
  const flickers = [60, 120, 200, 280, 400];
  flickers.forEach((ms, i) => {
    setTimeout(() => {
      if (!this._sanityOverlay) return;
      this._sanityOverlay.style.opacity = i % 2 === 0 ? '1' : '0';
    }, ms);
  });
  setTimeout(() => {
    if (this._sanityOverlay) this._sanityOverlay.style.opacity = '0';
  }, 500);

  // lights die for a moment
  this._torchLights.forEach(t => {
    t.light.intensity = 0;
    setTimeout(() => { t.light.intensity = t.base; }, 400 + Math.random()*200);
  });
}
...
if (this._dodgeActive) {
  ...
  this._iframes -= dt;
  const dSpeed = 9.5;
  this.camera.position.x += this._dodgeDir.x * dSpeed * dt;
  this.camera.position.z += this._dodgeDir.z * dSpeed * dt;
  if (this._dodgeTimer <= 0) this._dodgeActive = false;
}
if (this._dodgeCooldown > 0) this._dodgeCooldown -= dt;

// parry window opens during boss telegraph
if (this._currentAttack && this._attackPhase === 'telegraph') {
  this._parryWindow = 0.4; // window is always open during telegraph
} else {
  this._parryWindow = 0;
}
    ...
    if (this._currentAttack) {
      ...
      if (this._currentAttack === 'lunge' && this._attackPhase === 'active') {
        const lungeSpeed = this._phase2 ? 22 : 16;
    ...
    // ── MOVEMENT
    if (!this._bossStunned) {
      const baseSpeed = this._phase2 ? 4.2 : 2.8;
...
if (this._vignetteOverlay) {
  const danger = 1 - (this._playerHealth / 5);
  const vigOpacity = 0.4 + danger * 0.55;
  this._vignetteOverlay.style.opacity = `${vigOpacity}`;
}

// random sanity flicker events — more frequent in phase 2
this._sanityTimer = (this._sanityTimer || 12) - dt;
if (this._sanityTimer <= 0) {
  this._sanityTimer = this._phase2
    ? 4 + Math.random() * 5
    : 9 + Math.random() * 8;
  this._triggerSanityFlicker();
}

// boss watches you while idle — head tracks slowly even at distance
if (!this._currentAttack && !this._bossStunned && dist > 6) {
  // slow menacing approach instead of charging
  const lurk = this._phase2 ? 0.4 : 0.25;
  enemy.position.x += (dx / dist) * lurk * dt;
  enemy.position.z += (dz / dist) * lurk * dt;
}

// weapon leaves light trail in phase 2
if (this._phase2 && this._weaponLight) {
  this._weaponLight.intensity = 1.2 + Math.sin(t * 6) * 0.8;
}

// heartbeat screen pulse at 1 HP
if (this._playerHealth === 1) {
  const pulse = Math.sin(t * 4.5);
  if (this._vignetteOverlay && pulse > 0.95) {
    this._vignetteOverlay.style.opacity = '1.0';
  }
  // subtle red tint throb
  if (pulse > 0.98 && this._flashOverlay) {
    this._flashOverlay.style.background = 'rgba(120,0,0,0.25)';
    setTimeout(() => {
      if (this._flashOverlay) this._flashOverlay.style.background = 'rgba(0,0,0,0)';
    }, 80);
  }
}
// ambient pulse during boss fight
if (this._ambientBossPulse) {
  const bossIntensity = this._phase2 ? 1.2 : 0.5;
  this._ambientBossPulse.intensity = bossIntensity * (0.6 + 0.4 * Math.sin(t * 3.5));
  this._ambientBossPulse.position.set(enemy.position.x, 4, enemy.position.z);
  // fog thickens in phase 2
  if (this._phase2 && this.scene.fog) {
    this.scene.fog.density = Math.min(0.022, this.scene.fog.density + dt * 0.001);
  }
}

// stagger recovery
if (this._bossStaggerTimer > 0) {
  this._bossStaggerTimer -= dt;
  if (this._bossStaggerTimer <= 0) {
    this._enemy.rotation.x = 0;
    this._enemy.rotation.z = 0;
  }
  ...
  _reset() {
    ...
    this._swordGroup.rotation.set(0.1, -0.2, 0.05);
    this._swordGroup.position.set(0.36, -0.30, -0.65);
    if (this._vignetteOverlay) this._vignetteOverlay.style.opacity = '0.4';
if (this._sanityOverlay)   this._sanityOverlay.style.opacity = '0';
    // clear projectiles
